...
void TimeRestaurant::insertResTree(ResDataAVL& tree)
{
...
bool TimeRestaurant::reSize(Date& userDate) // if exceded the size, resize
{
    ...
    if (diffrence > 0) // so the users date is out of bound, should resize
    {
        ...
        for (int i = 0; i < maxSize; i++)
        {
            ...
            newArray[i].AVLtree = aptr[i].AVLtree;
        }
...
void TimeRestaurant::getSales()
{
    ...
    if (choice == 1) // sales of a month
    {
        ...
        theRestaurant = &aptr[getPosition(d)].AVLtree.search(name);
    ...
    if (choice == 2) // sales of a period of time
    {
        ...
        if (i < j) // the period of time is in multiple months
        {
            while (i != j)
            {
                theRestaurant = &aptr[i].AVLtree.search(name);
...
void TimeRestaurant::getDailySales(string name, Date date)
{
    ...
    theRestaurant = &aptr[getPosition(date)].AVLtree.search(name);
    ...
    cout << "The daily sales of: " << name << endl
        ...
        << "Total daily sales: " << algerian + syrian + chinese + indian + european << " DA\n";
}
...
{
    double oldCos;
    ResData* TheRestaurant = &aptr[getPosition(d)].AVLtree.search(name);
...
void TimeRestaurant::setTodaySale(string name, Date d, double sale, string choice)
{
    ...
    {                                        // should insert a bst in next current date->month cell
        aptr[position].AVLtree = aptr[getPosition(getLastDate())].AVLtree;
    }
    // if the restaurant is not present then insert it in the position
    if (!aptr[position].AVLtree.ContainsRes(name))
    {
        ResData Rest;
        Rest.NameR = name;
        aptr[position].AVLtree.InsertRes(Rest);
    }
...
// place related
void TimeRestaurant::getRatio(wilayaAVL mywilaya)
{
...
void TimeRestaurant::RatioCity(Date start, Date end, string cit, string wil, wilayaAVL country)
{
    double val = 0;
    int counter = 0;
    if (country.doesExistCity(wil, cit)) // if the city exists
    {
        ...
        vector<wilayaAVL::arrayCaseRList> districts = country.getCityRestaurants(wil, cit);
...
void TimeRestaurant::RatioWilaya(Date start, Date end, string wilayaName, wilayaAVL country)
{
    double val = 0;
    int counter = 0;
    if (country.doesExistWilaya(wilayaName))
    {
        vector<wilayaAVL::arrayCase1To2> cities = country.getWilayaRestaurants(wilayaName);
...
void TimeRestaurant::RatioCountry(Date start, Date end, wilayaAVL country)
{
    double val = 0;
    int counter = 0;
...
{
    double sal=0.0, pub=0.0;
    if (d.month == d2.month && d.year == d2.year) // here we're in the same month
    {
        ResData* theRestaurant = &aptr[getPosition(d)].AVLtree.search(name); // locate the restaurant
...
double TimeRestaurant::districtSales(string wil, string cit, string dist, const wilayaAVL::RestauNode* restoList)
{
    double sal = 0.0;
    ResData* theRestaurant;
    ...
    while (restoList != nullptr) // while not at the end of the list
    {
        for (int i = 0; i < position; i++) // from the start date to the end date in the aptr array
        {
            ...
            if (aptr[i].AVLtree.ContainsRes(name)) // if the restaurant is present calculate the sales for each cuisine
            {
                theRestaurant = &aptr[i].AVLtree.search(name);
...
void TimeRestaurant::districtSales(wilayaAVL mywilaya)
{
    string dist, cit, wil;
    ...
    const wilayaAVL::RestauNode* restoList = mywilaya.getDistrictRestaurants(wil, cit, dist); // the list of the resto in that district
    cout << "Sales of district: " << dist << districtSales(wil, cit, dist, restoList) << endl;
}

void TimeRestaurant::citySales(wilayaAVL mywilaya)
{
    string cit, wil;
    double sal = 0;
    ...
    vector<wilayaAVL::arrayCaseRList> districts = mywilaya.getCityRestaurants(wil, cit);
...
void TimeRestaurant::wilayaSales(wilayaAVL mywilaya)
{
    string wil;
    double sal = 0;
    ...
    vector<wilayaAVL::arrayCase1To2> cities = mywilaya.getWilayaRestaurants(wil);
...
void TimeRestaurant::countrySales(wilayaAVL mywilaya)
{
    double sal = 0;

    vector<wilayaAVL::arrayCase> wilayas = mywilaya.getCountryRestaurants();
    for (auto& element1 : wilayas)
    {
        cout << "wilaya: " << element1.regionName << endl;
        vector<wilayaAVL::arrayCase1To2> cities = *(element1.pointerToVector);
...
void TimeRestaurant::fillFromCuisineFiles()
{
    ...
    while (getline(file, line))
    {
        ...
        if (!aptr[getPosition(date)].AVLtree.ContainsRes(name)) // if the resto is not present insert it
        {
            ResData Rest;
            Rest.NameR = name;
            aptr[getPosition(date)].AVLtree.InsertRes(Rest);
        }

        ResData* therest = &aptr[getPosition(date)].AVLtree.search(name); // locate the resto
...
void TimeRestaurant::fillFromCostFile()
{
    ...
    while (getline(file, line))
    {
        ...
        if (!aptr[getPosition(date)].AVLtree.ContainsRes(name)) // if not found insert it
        {
            ResData Rest;
            Rest.NameR = name;
            aptr[getPosition(date)].AVLtree.InsertRes(Rest);
...
void TimeRestaurant::fillFromPublicityFile()
{
    ...
    while (getline(file, line))
    {
        ...
        if (!aptr[getPosition(date)].AVLtree.ContainsRes(name))
        {
            ResData Rest;
            Rest.NameR = name;
            aptr[getPosition(date)].AVLtree.InsertRes(Rest);
        ...
        ResData* therest = &aptr[getPosition(date)].AVLtree.search(name);
...
{
    ofstream outS;
    outS.open("sales2.txt", ofstream::out | ofstream::trunc);
    outS.close();
}
void TimeRestaurant::makeCostEmpty()
{
    ofstream outS;
    outS.open("cost2.txt", ofstream::out | ofstream::trunc);
    outS.close();
}
void TimeRestaurant::makePublicityEmpty()
{
    ofstream outS;
    outS.open("publicity2.txt", ofstream::out | ofstream::trunc);
    outS.close();
}

void TimeRestaurant::writeAllDataToFile() // from the constructed data write in files
{
    cout << "entered the write all to file " << endl;
    ofstream cuisineFile("sales2.txt", ios::app);
    ofstream costFile("cost2.txt", ios::app);
    ofstream PublicityFile("publicity2.txt", ios::app);
    ...
    Date d = getLastDate();
    cout << "last date: " << getLastDate().month << "/" << getLastDate().year << endl;
    int position = getPosition(d);
    cout << "position: " << position << endl;

    for (int i = 0; i <= position; i++)
    {
        writeCuisineToFile(cuisineFile, aptr[i].AVLtree.getRoot(), "algerian", i);
        writeCuisineToFile(cuisineFile, aptr[i].AVLtree.getRoot(), "syrian", i);
        writeCuisineToFile(cuisineFile, aptr[i].AVLtree.getRoot(), "chinese", i);
        writeCuisineToFile(cuisineFile, aptr[i].AVLtree.getRoot(), "indian", i);
        writeCuisineToFile(cuisineFile, aptr[i].AVLtree.getRoot(), "european", i);
        writeCostToFile(costFile, aptr[i].AVLtree.getRoot(), i);
        writePublicityToFile(PublicityFile, aptr[i].AVLtree.getRoot(), i);
    }
    costFile.close();
    PublicityFile.close();
    cuisineFile.close();
